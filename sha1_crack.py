...
def crack_sha1_hashfile():
# Defines a function that cracks sha1 hashes
    ...
    hash_file = input("Enter the full path to your hash file with the sha1 hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the sha1 hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=raw-sha1", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=raw-sha1", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=raw-sha1", "--incremental"]}
    ]

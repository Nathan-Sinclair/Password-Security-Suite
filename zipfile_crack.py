...
def crack_zipfile():
# Defines a function that cracks password protected zip files
    ...
    zip_file = input("Enter the full path to your password protected zip file: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the password-protected zip file and the wordlist
    ...
    try:
        ...
        zip2john_exe = os.path.join(john_path, "zip2john.exe")
        subprocess.run([zip2john_exe, zip_file], stdout=open(hash_file, "w"), check=True)

...
def crack_md5_hashfile():
# Defines a function that cracks md5 hashes
    ...
    hash_file = input("Enter the full path to your hash file with the MD5 hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the md5 hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=raw-md5", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=raw-md5", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=raw-md5", "--incremental"]}
    ]

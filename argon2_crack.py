...
def crack_argon2_hashfile():
# Defines a function that cracks argon2 hashes
    ...
    hash_file = input("Enter the full path to your hash file with the argon2 hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the argon2 hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=argon2", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=argon2", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=argon2", "--incremental"]}
    ]

...
def crack_bcrypt_hashfile():
# Defines a function that cracks bcrypt hashes
    ...
    hash_file = input("Enter the full path to your hash file with the bcrypt hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the bcrypt hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=bcrypt", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=bcrypt", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=bcrypt", "--incremental"]}
    ]

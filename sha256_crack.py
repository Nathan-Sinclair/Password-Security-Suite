...
def crack_sha256_hashfile():
# Defines a function that cracks sha256 hashes

    print("John The Ripper must be install on your computer")
    ...
    hash_file = input("Enter the full path to your hash file with the sha256 hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the sha256 hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=raw-sha256", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=raw-sha256", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=raw-sha256", "--incremental"]}
    ]

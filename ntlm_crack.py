...
def crack_ntlm_hashfile():
# Defining the function which will crack ntlm hashes
    ...
    hash_file = input("Enter the full path to your hash file with the NTLM hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the ntlm hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=NT", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=NT", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=NT", "--incremental"]}
    ]

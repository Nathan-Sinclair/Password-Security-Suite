...
def crack_scrypt_hashfile():
# Defines a function that cracks scrypt hashes
    ...
    hash_file = input("Enter the full path to your hash file with the scrypt hash: ").strip()
    wordlist_file = input("Enter the full path to your wordlist file: ").strip()
    # Prompts user to enter the full paths to the John The Ripper folder, the scrypt hash file and the wordlist
    ...
    stages = [
         {"desc": "Wordlist attack", "args": ["--format=scrypt", f"--wordlist={wordlist_file}"]},
         {"desc": "Wordlist + mutations (rules)", "args": ["--format=scrypt", f"--wordlist={wordlist_file}", "--rules"]},
         {"desc": "Brute force attack", "args": ["--format=scrypt", "--incremental"]}
    ]
